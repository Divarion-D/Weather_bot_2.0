...
class Diagram:
    ...
    def draw_diagram(self):
        ...
        Draw the diagram.
        '''
        self.ax1.vlines(x=self.days, ymin=0, ymax=self.humaditys, color='#89CFF0', linewidth=20) # Вертикальные линии
        ...
        for i, self.humaditys in enumerate(self.humaditys):
            self.ax1.text(i+1, self.humaditys+0.5, round(self.humaditys, 1), horizontalalignment='center')

        self.ax2.vlines(x=self.days, ymin=0, ymax=self.temperatures, color='#FF0000', linewidth=20) # Вертикальные линии
        self.ax2.set_ylim(self.min_temperature-10, self.max_temperature+10) # Установка границ по оси Y
        ...
        self.ax2.set_yticks(self.temperature_key) # Установка осей для температуры
        ...
        for i, self.temperatures in enumerate(self.temperatures):
            if self.temperatures > 0:
                ...
                self.ax2.text(i+1, -8, round(self.temperatures, 1), horizontalalignment='center') 
            else:
                # Если температура меньше 0, то показываем ее внизу
                self.ax2.text(i+1, 2, round(self.temperatures, 1), horizontalalignment='center')
...
if __name__ == '__main__':
    ...
    temperatures = np.random.randint(-50, 50, size=31) # Массив случайных чисел
